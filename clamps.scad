...
module clampshape1half(s1=3.7, y1=17.5) {
             ...
             polygon(points=[[0,0-m],[s2-0.3+m,0-m],[s2-0.1+m,0.1-m],[s2+m,0.3-m],[s2+0.1+m,0.5-m],[s2+m,1-m],
...
module clampshape2(clampwidth=3.7, clamplength=17.5, cz=20) {
     ...
     union() {
       clampshape1();
       translate([0,-w,cz/2]) rotate([90,90,0]) linear_extrude(height=0.3) text(str(clampwidth), size = fontsize, font = fontname, halign = "center", valign = "center", $fn = 32);
     }
}
...
module clampshape3(clampwidth=3.7, clamplength=17.5, cz=20) {
     ...
     union() {
         clampshape2();
         translate([0,-w,  cz/4-m]) rotate(180) rotate([0,180]) dovetail3D(h=cz/4, s=45, t=m);
         translate([0,-w,3*cz/4+m]) rotate(180) rotate([0,  0]) dovetail3D(h=cz/4, s=45, t=m);
     }       
}
